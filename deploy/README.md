...
### 显存核算 (已实测, 4060 bf16+checkpoint, 含梯度+AdamW)

| 模型 | batch | 峰值显存 | 4090 16GB 余量 |
|---|---|---|---|
| VAE (9.25M) | 4 | 3.8 GB | ~12 GB |
| VAE | 8 | 7.5 GB | ~8.5 GB (deploy 默认) |
| VAE | 12 | ~11.4 GB (线性外推) | ~4.5 GB |
| 扩散 (117M) | 8 | 3.1 GB | ~13 GB (deploy 默认) |
| 扩散 | 16 | ~6 GB (线性外推) | ~10 GB |

- 线性规律: VAE ~0.93GB/样本, 扩散 ~0.35GB/样本 (checkpoint 后激活被重算)
- DDP 双卡 = 每卡一份参数/梯度/优化器, 激活按各卡 batch, 峰值不叠加
- 安全红线: 单卡峰值 ≤ 12GB (留 ~4GB 给别人的常驻线程/驱动)
- 余量充足可调大 batch: VAE ≤12, 扩散 ≤16; 保守就用默认
